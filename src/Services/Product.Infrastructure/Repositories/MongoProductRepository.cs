...
internal sealed class MongoProductRepository(ProductMongoDbContext context) : IProductRepository
{
    ...
    private static FilterDefinition<ProductDocument> ActiveOnly() =>
        Builders<ProductDocument>.Filter.Eq(d => d.IsActive, true);
        ...
        var filter = string.IsNullOrWhiteSpace(search)
            ...
            : Builders<ProductDocument>.Filter.And(
                ...
                Builders<ProductDocument>.Filter.Or(
                    Builders<ProductDocument>.Filter.Regex(d => d.Name, new MongoDB.Bson.BsonRegularExpression(search, "i")),
                    Builders<ProductDocument>.Filter.Regex(d => d.Description, new MongoDB.Bson.BsonRegularExpression(search, "i"))));
    ...
    public async Task<bool> ExistsAsync(ProductId id, CancellationToken ct = default) =>
        await _collection.CountDocumentsAsync(ById(id), cancellationToken: ct) > 0;
    ...
    private static ProductDocument MapToDocument(DomainProduct p) => new()
    {
        Id = p.Id.Value,
        Name = p.Name,
